#!/usr/bin/env python3
"""
Test script for minimal server connectivity
"""
...
def test_chat():
    try:
        ...
        response = requests.post(
            "http://localhost:8001/api/chat/message/json",
            json=payload,
...
if __name__ == "__main__":
    print("Testing minimal server connectivity on port 8001...")
    ...
    if root_ok and health_ok and chat_ok:
        print("\n✅ All tests passed! Minimal server is working.")
    else:
        print("\n❌ Some tests failed.")
        if not any([root_ok, health_ok, chat_ok]):
            print("💡 Server might not be running or accessible on port 8001")